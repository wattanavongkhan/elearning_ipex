...
<div class="max-w-12xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('lessons.update', $lesson->id) }}" method="POST" enctype="multipart/form-data"
            ...
            @method('PUT')
            <div class="mb-8">
                <h3 class="text-xl font-bold text-gray-700">แก้ไขบทเรียน : <span
                        class="text-blue-600">{{ $lesson->title }}</span></h3>
                <nav class="text-sm text-gray-500 mb-1">
                    <a href="#" class="hover:text-blue-600 transition">รายละเอียดบทเรียน</a>
                </nav>
            ...
            <div class="grid grid-cols-1 gap-y-6">
                ...
                <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    ...
                    <div class="grid grid-cols-1 gap-y-4">
                        <div>
                            <label for="video_url" class="block text-xs font-semibold text-gray-600 mb-1">Link วิดีโอ
                                (Vimeo)</label>
                        ...
                        <div class="relative py-2">
                            <div class="absolute inset-0 flex items-center"><span
                                    class="w-full border-t border-gray-200"></span></div>
                            <div class="relative flex justify-center text-xs uppercase"><span
                                    class="bg-slate-50 px-2 text-gray-400 font-bold">หรือเปลี่ยนไฟล์วิดีโอ</span></div>
                        </div>

                        <div>
                            <label for="video_file"
                                class="block text-xs font-semibold text-gray-600 mb-1">อัปโหลดไฟล์วิดีโอใหม่
                                (MP4)</label>

                            @if($lesson->video_path)
                            <div
                                class="mb-3 flex items-center gap-2 p-3 bg-blue-50 rounded-xl border border-blue-100 w-fit">
                                <i class="fas fa-file-video text-blue-600"></i>
                                <span
                                    class="text-xs text-blue-800 font-medium">ปัจจุบันมีไฟล์วิดีโออัปโหลดไว้แล้ว</span>
                            </div>
                            @endif
                            ...
                            <p class="text-[10px] text-gray-400 mt-2">* เว้นว่างไว้หากไม่ต้องการเปลี่ยนไฟล์วิดีโอ</p>
                        </div>
                    </div>
                </div>

                <div>
                    <label for="content" class="block text-sm font-semibold text-gray-700 mb-2">เนื้อหาเพิ่มเติม /
                        คำอธิบาย</label>
                    <textarea name="content" id="content" rows="4"
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
                        placeholder="รายละเอียดหรือลิงก์ดาวน์โหลดเอกสารประกอบการเรียน...">{{ old('content', $lesson->content) }}</textarea>
                </div>
