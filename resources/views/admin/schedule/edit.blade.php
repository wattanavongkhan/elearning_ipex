                ...
                <div class="grid grid-cols-1 gap-y-6">
                    <div class="mb-4 grid grid-cols-1 md:grid-cols-4 gap-7">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="purpose" id="purpose" value="{{ old('purpose', $calendarEvent->purpose) }}" required
                        ...
                        <div class="md:col-span-1">
                            ...
                            <input type="datetime-local" name="start_date" id="start_date"
                                value="{{ old('start_date', $calendarEvent->start_date) }}" required class="w-full border rounded-lg p-2">
                        </div>
                        <div class="md:col-span-1">
                            <label for="end_date" class="block text-sm font-semibold text-gray-700 mb-2">End Date <span
                                    class="text-red-500">*</span></label>
                            <input type="datetime-local" name="end_date" id="end_date" value="{{ old('end_date', $calendarEvent->end_date) }}"
                ...
                <div class="mb-4 grid grid-cols-1 md:grid-cols-2 gap-7">
                    <div>
                        ...
                        <select name="room_id" class="w-full border rounded-lg p-2">
                            ...
                            <option value="{{ $room->id }}" {{ old('room_id', $calendarEvent->room_id) == $room->id ? 'selected' : '' }}>
                                {{ $room->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select class="w-full border rounded-lg p-2" name="category_color" id="category_color">
                            <option value="#3b82f6" {{ old('category_color', $calendarEvent->category_color) == '#3b82f6' ? 'selected' : '' }}>Blue </option>
                            <option value="#10b981" {{ old('category_color', $calendarEvent->category_color) == '#10b981' ? 'selected' : '' }}>Green</option>
                            <option value="#f59e0b" {{ old('category_color', $calendarEvent->category_color) == '#f59e0b' ? 'selected' : '' }}>Yellow</option>
                            <option value="#ef4444" {{ old('category_color', $calendarEvent->category_color) == '#ef4444' ? 'selected' : '' }}>Red</option>
                        </select>
                    </div>
                </div>
                <div class="mb-4">
                    <label class="block font-bold mb-2">Members</label>
                    <textarea name="members" class="w-full border rounded-lg p-2">{{ old('members', $calendarEvent->members) }}</textarea>
                </div>
