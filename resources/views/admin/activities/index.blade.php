...
<div class="max-w-12xl mx-auto">
    <div class="bg-white rounded-[1rem] shadow-sm border border-slate-100 overflow-hidden p-5">
        ...
        <table class="w-full text-left border-collapse" id="activity_tb">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                    <th class="px-6 py-5 text-slate-400 font-bold text-xs uppercase tracking-widest">ข้อมูล</th>
                    <th class="px-6 py-5 text-slate-400 font-bold text-xs uppercase tracking-widest">ประเภท/วันที่จัด
                    </th>
                    <th class="px-6 py-5 text-slate-400 font-bold text-xs uppercase tracking-widest text-center">
                        ยอดเข้าชม
                    </th>
                    <th class="px-6 py-5 text-slate-400 font-bold text-xs uppercase tracking-widest text-center">สถานะ
                    </th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="relative">
                                <img src="{{ asset('storage/' . $item->thumbnail) }}"
                                    class="w-14 h-14 rounded-2xl object-cover shadow-sm border border-slate-100">
                                @if($item->is_featured)
                                <span
                                    class="absolute -top-2 -left-2 bg-amber-400 text-white w-6 h-6 rounded-full flex items-center justify-center border-2 border-white shadow-sm"
                                    title="ปักหมุด">
                    ...
                    <td class="px-6 py-5">
                        ...
                        <div
                            class="inline-flex items-center bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase mb-1">
                            <i class="fas fa-calendar-day mr-1"></i> Activity
                        </div>
                        <div class="text-xs font-bold text-slate-600">
                            {{ $item->event_date ? $item->event_date->format('d M Y') : '-' }}</div>
                        @else
                        <div
                            class="inline-flex items-center bg-blue-50 text-blue-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase">
                    ...
                    <td class="px-6 py-5 text-center">
                        <span class="text-sm font-black text-slate-700"><i class="fas fa-eye text-slate-300 mr-1"></i>
                    ...
                    <td class="px-6 py-5 text-center">
                        @if($item->status == 1)
                        <span
                            class="text-green-500 bg-green-50 px-3 py-1 rounded-full text-[10px] font-black">PUBLISHED</span>
                        @else
                        <span
                            class="text-slate-400 bg-slate-100 px-3 py-1 rounded-full text-[10px] font-black">DRAFT</span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-5 text-right">
                        <div class="flex justify-end gap-2">
                            ...
                            <form action="{{ route('admin.activities.destroy', $item->id) }}" method="POST"
                                class="inline" onsubmit="return confirm('คุณต้องการลบรายการนี้ใช่หรือไม่?')">
                                @csrf @method('DELETE')
                                <button
