                        ...
                        <div class="space-y-3">
                            ...
                            <label
                                ...
                                <div class="aspect-video w-20 rounded-xl overflow-hidden bg-slate-100 mr-5">
                                    <img src="{{ asset($option_images[$key]) }}" class="w-20 transition-transform">
                                </div>
                                @endif
    ...
    document.getElementById('quiz-form').onsubmit = function (e) {
        ...
        fetch("{{ route('quiz.submit') }}", {
            ...
            .then(data => {
                ...
                //console.log('Passed:', data.passed, 'Less_st:', '{{$less_st}}', 'Progress Percent:', '{{$progress_percent}}');
                if (data.passed && data.progress_percent == "100") 
                {
                    icon.className =
                    ...
                    text.innerText = "คุณทำคะแนนได้ยอดเยี่ยม จบหลักสูตรนี้แล้ว ตอนนี้คุณสามารถเข้าเรียนต่อคอร์สอื่นๆ ได้แล้ว";
                    btnAction.innerText = "ไปที่หน้าคอร์สเรียน";
                    ...
                    btnAction.onclick = () => window.location.href =
                        "{{ route('courses.learn', ['id' => $course_id, 'lesson_id' => $nextlesson]) }}";
